...
class PageGenerator:
    ...
    def __init__(
        ...
        # Project root for finding data files (independent of output_dir depth)
        ...
        logger.info(
            f"Page generator initialized (templates: {template_dir}, output: {output_dir}, update: {update_prefix or 'none'})"
        )

    def _get_common_context(self, app_version: str = "1.0.0") -> Dict[str, Any]:
        """Get context variables common to all templates."""
        ctx = {
        ...
        if self.update_prefix:
            ctx["update_prefix"] = self.update_prefix
            ctx["updates_page_url"] = "../updates.html"
        return ctx

    def generate_build_page(
        self, build: CommonBuild, update_version: str, app_version: str = "1.0.0"
    ) -> str:
        """
        ...
        if build.best_player:
            logger.debug(
                f"Passing to template - Best player: {build.best_player.character_name}, DPS: {build.best_player.dps:,}"
            )
    ...
    def generate_home_page(
        ...
        trials_file = self.project_root / "data" / "trials.json"
        ...
        for trial_name in all_trial_names:
            ...
            if trial_name in builds_by_trial:
                ...
                # Exclude trash builds - only select from actual boss fights
                for boss_name, boss_data in bosses.items():
                    # Skip trash builds
                    if boss_name == "Trash Builds":
                        continue
            ...
            # Also ensure we're not including any trash builds that might have slipped through
            dps_builds = [
                b
                for b in all_builds
                if b.best_player
                and b.best_player.role == "dps"
                and b.boss_name != "Trash Builds"
            ]
            ...
            trials.append(
                {
                    ...
                    "has_data": trial_name in trials_with_builds
                    and top_build is not None
                    and len(all_builds) > 0,  # Track if trial has any data
                }
            )
    ...
    def generate_router_page(self, app_version: str = "1.0.0") -> str:
        """
        Generate a root router page that redirects to the active update.
        ...
        Returns:
            Path to generated HTML file
        """
        logger.info("Generating root router page")

        config_path = self.project_root / "data" / "update_config.json"
        with open(config_path, "r", encoding="utf-8") as f:
            update_config = json.load(f)

        active_update = update_config["active_update"]
        active_update_config = update_config.get("updates", {}).get(active_update, {})
        active_prefix = active_update_config.get("path_prefix", active_update)
        redirect_url = f"{active_prefix}/index.html"
        active_update_label = active_update_config.get("label", active_update.upper())

        template = self.env.get_template("router.html")
        context = {
            **self._get_common_context(app_version),
            "redirect_url": redirect_url,
            "active_prefix": active_prefix,
            "active_update_label": active_update_label,
        }
        html = template.render(**context)

        filepath = self.output_dir.parent / "index.html"
        filepath.write_text(html, encoding="utf-8")

        logger.info(f"Generated root router page: {filepath}")
        return str(filepath)
    ...
    def generate_trial_page(
        ...
        for boss_name, builds in bosses.items():
            ...
            sorted_builds = sorted(
                ...
                key=lambda b: (
                    role_order.get(
                        b.best_player.role.lower() if b.best_player else "dps", 3
                    ),  # Role order (unknown roles last)
    ...
    def generate_updates_page(self, app_version: str = "1.0.0") -> str:
        ...
        Returns:
            Path to generated HTML file
        """
        logger.info("Generating updates directory page")

        update_config_file = self.project_root / "data" / "update_config.json"
        with open(update_config_file, "r") as f:
            update_config = json.load(f)

        active_update = update_config.get("active_update", "")
        updates_config = update_config.get("updates", {})

        updates: List[Dict[str, str]] = []
        for update_data in updates_config.values():
            updates.append(
                {
                    "label": str(update_data.get("label", "")),
                    "display_name": str(update_data.get("display_name", "")),
                    "path_prefix": str(update_data.get("path_prefix", "")),
                    "status": str(update_data.get("status", "pending")),
                }
            )

        template = self.env.get_template("updates_directory.html")
        context = {
            "updates": updates,
        ...
        filepath = self.output_dir.parent / "updates.html"
        filepath.write_text(html, encoding="utf-8")

        logger.info(f"Generated updates directory page: {filepath}")
        return str(filepath)
    ...
    def generate_tldr_summary_page(
        ...
        for role in sorted(
            aggregated_builds.keys(), key=lambda r: role_order.get(r, 3)
        ):
        ...
        context = {
            **self._get_common_context(app_version),
            ...
            "bosses": bosses,
            "social_image_url": self._get_social_image_url(
                "trial", "Top Builds", app_version
            ),
            "hide_download_button": csv_filename
            is None,  # Hide only if CSV wasn't generated
    ...
    def generate_aggregated_build_page(
        self, build: CommonBuild, update_version: str, app_version: str = "1.0.0"
    ) -> str:
        """
    ...
    def generate_all_pages(
        ...
        home_path = self.generate_home_page(
            builds_by_trial, trials_metadata, app_version, aggregated_builds
        )
        ...
        for trial_name, trial_data in builds_by_trial.items():
            ...
            generated_files[f"trial_{trial_name.lower().replace(' ', '_')}"] = (
                trial_path
            )
            ...
            try:
                csv_path = self.csv_exporter.export_trial_data_from_builds(
                    trial_name=trial_name, builds_by_boss=bosses_data
                )
    ...
    def generate_sitemap(
        ...
        base_url = (
            "https://brainsnorkel.github.io/eso-build-o-rama"
            if self.is_develop
            else "https://esobuild.com"
        )
        if self.update_prefix:
            base_url = f"{base_url}/{self.update_prefix}"
    ...
    def generate_sitemap_index(self, app_version: str = "1.0.0") -> str:
        """
        Generate sitemap index XML file referencing all update sitemaps.

        This creates the root-level sitemap-index.xml that search engines
        use to discover sitemaps for all published updates.

        Args:
            app_version: Application version for logging

        Returns:
            Path to generated sitemap-index.xml file (root directory)
        """
        logger.info("Generating sitemap-index.xml")

        # Read update config to find all active/archived updates
        config_file = self.project_root / "data" / "update_config.json"
        try:
            with open(config_file, "r") as f:
                config = json.load(f)
        except (FileNotFoundError, json.JSONDecodeError) as e:
            logger.warning(f"Could not load update_config.json: {e}")
            config = {"updates": {}}
        ...
        # Add sitemap entry for each active/archived update
        updates = config.get("updates", {})
        for update_key, update_data in sorted(updates.items()):
            status = update_data.get("status", "")
            # Only include active and archived updates, skip pending
            if status in ("active", "archived"):
                path_prefix = update_data.get("path_prefix", "")
                xml_lines.extend(
                    [
                        "  <sitemap>",
                        f"    <loc>https://esobuild.com/{path_prefix}/sitemap.xml</loc>",
        ...
        # Write sitemap index to root output directory
        xml_content = "\n".join(xml_lines)
        filepath = self.output_dir.parent / "sitemap-index.xml"
        filepath.write_text(xml_content, encoding="utf-8")

        logger.info(f"Generated sitemap-index.xml: {filepath}")
        return str(filepath)

    def generate_root_robots_txt(self, app_version: str = "1.0.0") -> str:
        """
        Generate root-level robots.txt pointing to sitemap index.

        This creates the root-level robots.txt that applies to all updates
        and directs search engine crawlers to the sitemap index.

        Args:
            app_version: Application version for logging

        Returns:
            Path to generated robots.txt file (root directory)
        """
        logger.info("Generating root robots.txt")

        robots_content = """# robots.txt for ESOBuild.com
User-agent: *
Allow: /

# Sitemap index location
Sitemap: https://esobuild.com/sitemap-index.xml

# Disallow cache directory (if exposed)
Disallow: /cache/
"""

        # Write robots.txt to root output directory
        filepath = self.output_dir.parent / "robots.txt"
        filepath.write_text(robots_content, encoding="utf-8")

        logger.info(f"Generated root robots.txt: {filepath}")
        return str(filepath)
...
"""
    ...
    def _group_builds_by_trial(
        ...
        for build in builds:
            ...
            if boss not in grouped[trial]:
                grouped[trial][boss] = {"builds": [], "total_reports": 0}
    ...
    def _get_social_image_url(
        ...
        if self.update_prefix:
            base_url = f"https://esobuild.com/{self.update_prefix}/"
        else:
        ...
        if page_type == "trial" and trial_name:
            ...
            trial_slug = (
                trial_name.lower().replace(" ", "").replace("-", "").replace("'", "")
            )
            filename = (
                f"social-preview-{trial_slug}-dev-{image_version}.png"
                if self.is_develop
                else f"social-preview-{trial_slug}-{image_version}.png"
            )
        elif page_type == "build":
            # Use site banner for build pages
            filename = (
                f"social-preview-build-dev-{image_version}.png"
                if self.is_develop
                else f"social-preview-build-{image_version}.png"
            )
        else:
            # Use site banner for home page
            filename = (
                f"social-preview-dev-{image_version}.png"
                if self.is_develop
                else f"social-preview-{image_version}.png"
            )
